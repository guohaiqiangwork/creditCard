	...
	<view>
		<view v-if="getWxNumberF" class="uni-flex background_colorff padding_bottom3 padding_top3 border_bottom">
			<view class="font_size30 margin_left3  width70">
				...
				<image src="../../static/image/icon/rightb.png" class="margin_left2" style="width: 8upx;height: 14upx;margin-top: 20upx;" mode=""></image>
			</view>
			<view class="font_size30 text_center width30 " @click="open_moudel">提现说明</view>
		</view>
		<view class="uni-flex  padding_bottom3 padding_top3 " v-if="!getWxNumberF">
			<view class="font_size30 margin_left3  width70 " style="font-weight: 700;">
				微信账号
				<text class="margin_left3" style="font-weight: 700;">{{ infoData.wxNumber }}</text>
			</view>
			<view class="font_size30 text_center width30 " @click="open_moudel">提现说明</view>
		</view>
			...
			<view class="with_moudel">
				<view class="border_bottom ">
					...
					<view class="uni-flex font_size50">
						<view class="font_size50 width10">¥</view>
				...
				<view class="uni-flex margin_top3">
					...
					<view class="font_size26 font_color99">手续费:￥{{ SxfFalg }}%</view>
				</view>

				<view class="tx_btn" @click="open_password">提现</view>

				<!-- 支付密码 -->
				<template v-if="payFalg">
					<view class="moudel_content">
						<view :class="setFalg ? 'content_block' : 'content_block_pay'">
							<view class="uni-flex uni-row margin_left3 margin_top3" v-if="!setFalg">
								<view class="width30" @click="close_password"><image src="../../static/image/icon/colse.png" mode="" class="moudel_img"></image></view>

								<view class="font_color33 font_size30">请输入提现密码</view>
							</view>
							<view class="uni-flex uni-row margin_left3 margin_top3" v-else>
								<view class="width30" @click="close_password"><image src="../../static/image/icon/colse.png" mode="" class="moudel_img"></image></view>

								<view class="font_color33 font_size30 margin_left5">设置提现密码</view>
							</view>
							<view class="font_color33 font_size30 text_center" v-if="setFalg">支付密码为6位数字</view>
							<view class="font_color33 font_size30 text_center" v-if="!setFalg">提现</view>
							<view class="font_color33 font_size60 text_center" v-if="!setFalg">￥{{ money }}</view>
							<view class="font_size26 text_center" style="color: #F34342;" v-if="!setFalg">*其中扣减：手续费{{ moneyTx }}</view>
							<view class="margin_top5"><validcode :maxlength="6" :isPwd="true" @finish="getPwd" ref="pwd"></validcode></view>
							<view class="padding_bottom3"><view class="moudel_btn" v-if="setFalg" @click="funSetPassword">确认</view></view>
						</view>
					</view>
				</template>
					...
					<view class="moudel_content">
						<view class="explain_moudel">
							<view class="uni-flex margin_left3 margin_top3">
								<view class="width35" @click="close_moudel"><image src="../../static/image/icon/colse.png" class="moudel_img_one" mode=""></image></view>
								<view class="font_size34">提现说明</view>
							</view>
							<view class="font_size22 margin_left3 margin_right3 font_size28 margin_top3">
								一、提现相关操作 1.用户在56金服平台，进入账户余额-点击提现-填写提现金额-选择是否使用提现券，进入填写存管银行提现页面，正确填写交易密码后即可提现成功
								2.用户账户有可用资金即可提现，两个小时内到账。 A.5万（含）以下提现（24小时）两个小时内到账；
								B.5万以上提现工作日工作时间（8:00-17:00）两个小时内到账； C.5万以上非工作日工作时间为次工作日到账； 3. 温馨提示
								夜间/节假日期间如有大额提现（单笔5万以上）需求，建议采用多笔小额操作 4. 由于银行清算次日到账，当天新充值的资金可用于投资，提现需次日才可提现。
							</view>
						</view>
					</view>
				</template>
...
export default {
	...
	methods: {
		...
		passwordCheck() {
			...
			this.$http
				.post('/acc/passwordCheck', pData, true)
				.then(res => {
					console.log(JSON.stringify(res));
					if (res.data.code == 200) {
		...
		goToR() {
			...
			this.$http
				.post('/acc/reflectWxblance', data, true)
				.then(res => {
					console.log(JSON.stringify(res));
					if (res.data.code == 200) {
						this.explainFalg = false;
		...
		open_password() {
			if (this.getWxNumberF) {
				...
				return;
			} else if (Number(this.moneyValueF) <= 0 || !this.moneyValueF) {
				uni.showToast({
					title: '请填写金额',
					icon: 'none',
					duration: 2000,
				...
				return;
			} else if (Number(this.moneyValueF) > Number(this.yeFalg)) {
				uni.showToast({
					title: '提现金额大于余额',
					icon: 'none',
					duration: 2000,
			...
			this.$http.get('/acc/isSetPassword/' + uni.getStorageSync('memberId')).then(res => {
				...
				if (res.data.code == 500) {
					...
					this.moneyTx = this.accMul(Number(this.moneyValueF), Number(this.moneyFalg));
				}
			});
		},
		...
		funGetWxNumber: function() {
			this.$http
				.get('/mb/getWxNumber/' + uni.getStorageSync('memberId'))
				.then(res => {
					if (res.data.code != 200) {
